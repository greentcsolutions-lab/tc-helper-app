...
export function UpgradeToStandardButton({ size = 'default' }: { size?: 'default' | 'lg' | 'sm' }) {
  const [loading, setLoading] = useState(false);

  const handleUpgrade = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/subscriptions/checkout-standard', {
        method: 'POST',
      });
  ...
  return (
    ...
    >
      <Zap className="h-4 w-4" />
      {loading ? 'Loading...' : 'Upgrade to Standard'}
    </Button>
  );
}
