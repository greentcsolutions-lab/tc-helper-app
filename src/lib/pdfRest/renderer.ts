...
// DPI CONTROL FOR FINAL PNG IMAGES
// Change the 'resolution' form field below to experiment (string value, 12–2400 DPI supported by pdfRest /png endpoint)
// Current default: "320" — sweet spot for Grok vision accuracy (crisp text/handwriting) vs token cost/size/speed
// Examples to try:
// - "290" → original setting (fastest, good enough for most)
// - "350" → sharper handwriting detection
// - "400" → even crisper, slight cost/size increase
// - "200" → lower quality/faster/cheaper (test for fallback)
...
export async function renderPdfToPngBase64Array(
  ...
  // <<< DPI SETTING HERE — tweak this value to play with final image quality >>>
  const TARGET_DPI = "290"; // ← Change this string to any integer 12–2400
  // <<< END DPI SETTING >>>

  console.log("[pdfRest] Starting PDF → PNG conversion", { 
    maxPages: maxPages || "all",
    resolutionDPI: TARGET_DPI 
  });
  ...
  form.append("resolution", TARGET_DPI); // ← This controls final PNG DPI
  ...
  try {
    ...
    console.log(`[pdfRest] SUCCESS: Returned ${filteredPages.length} pages at ${TARGET_DPI} DPI`);
